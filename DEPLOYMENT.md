...
- MySQL 8.0+ (إذا لم تستخدم Docker)
...
```bash
# Backup MySQL
docker-compose exec mysql mysqldump -u annotator -p arab_annotators > backup.sql

# Restore MySQL
docker-compose exec -T mysql mysql -u annotator -p arab_annotators < backup.sql
```
...
# Verify connection
docker-compose exec mysql mysql -u annotator -p -e "SELECT 1"
```
