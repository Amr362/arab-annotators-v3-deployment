...
## تاريخ التحديث
**2026-05-13** — إصلاح مشاكل النشر والـ Healthcheck على Railway
...
### 4. تحسين Dockerfile والـ Healthcheck ✅

**المشكلة:**
- الـ Healthcheck كان يستخدم `node -e` مع `import('http')` مما قد يسبب مشاكل في بيئة Alpine.
- المنفذ كان مضبوطاً على 5000 بينما الخادم يميل لاستخدام 3000.
- غياب أداة `curl` في صورة الإنتاج.

**الإصلاح:**
- تم تثبيت `curl` في صورة الإنتاج.
- تحديث الـ Healthcheck ليستخدم `curl` وهو أكثر موثوقية.
- توحيد المنفذ على **3000** في `Dockerfile` و `railway.toml`.
- زيادة فترة السماح (`start-period`) للـ Healthcheck إلى 30 ثانية لإعطاء وقت للهجرات.

### 5. دعم DATABASE_PRIVATE_URL في Drizzle ❌ → ✅

**المشكلة:**
- ملف `drizzle.config.ts` كان يعتمد فقط على `DATABASE_URL` بينما توفر Railway أحياناً `DATABASE_PRIVATE_URL`.
- هذا كان يسبب فشل `pnpm db:push` عند بدء التشغيل.

**الإصلاح:**
- تحديث `drizzle.config.ts` ليدعم كلا المتغيرين.
- تحسين أمر `start` في `package.json` ليتخطى `db:push` بأمان إذا لم تتوفر قاعدة البيانات بدلاً من الانهيار.
