            ...
            <>
              <div className="flex-1 overflow-auto p-5 space-y-4">
                {pendingTasks.length === 0 ? (
                  ...
                  {/* Task nav bar - Simplified */}
	                  <div className="flex items-center justify-between bg-white rounded-2xl border border-slate-100 px-4 py-3 shadow-sm">
	                    <div className="flex items-center gap-4">
	                      {/* Timer */}
	                      <div className={cn("flex items-center gap-2 text-[15px] font-bold font-mono bg-slate-50 border border-slate-200 px-4 py-2 rounded-xl transition-colors", timer.color)}>
	                        <Timer size={16} />
	                        {timer.fmt}
	                      </div>
	                      ...
	                      <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-4 py-2 rounded-xl">
	                        <span className="text-xs text-slate-400 font-medium">إجمالي المهام:</span>
	                        <span className="text-[15px] font-bold text-slate-700 tabular-nums">
	                          {pendingTasks.length}
	                        </span>
	                        <div className="w-px h-4 bg-slate-200 mx-1" />
	                        <span className="text-xs text-slate-400 font-medium">المنجزة:</span>
	                    ...
	                    <div className="flex items-center gap-3">
	                      ...
	                      <button onClick={() => setShowSkipModal(true)}
	                        className="flex items-center gap-2 text-sm font-bold text-amber-600 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 px-4 py-2 rounded-xl transition-all shadow-sm">
	                        <SkipForward size={15} />
	                        تخطي المهمة
	                        <kbd className="hidden sm:inline-block text-[10px] bg-white border border-amber-200 px-1.5 rounded font-mono ml-1">S</kbd>
	                      </button>
	                    </div>
	                  </div>
